...
import phoneScreenMockup from '@/assets/mockups/phone-screen-mockup.webp';
...
/**
 * Composites the user's artwork onto the phone screen mockup (green-screen replacement).
 */
export default function PhoneScreenMockup({ artworkSrc, className = '' }) {
    ...
    (async () => {
      try {
        ...
        if (bounds) {
          ...
          // Replace only green pixels with artwork
          for (let i = 0; i < greenMask.length; i++) {
            if (greenMask[i]) {
              const o = i * 4;
        ...
        console.error('PhoneScreenMockup composite failed:', err);
      }
    })();
  ...
  return (
    <div className={`relative w-full overflow-hidden ${className}`}>
      <img
        src={composited || phoneScreenMockup}
        alt="Your artwork as a phone wallpaper"
        loading="eager"
