import { serve } from "https://deno.land/std@0.190.0/http/server.ts";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Methods": "POST, OPTIONS",
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type, x-supabase-client-platform, x-supabase-client-platform-version, x-supabase-client-runtime, x-supabase-client-runtime-version",
};

const VARIANT_MAP: Record<string, string> = {
  standard: "gid://shopify/ProductVariant/48006450938078",
  high_resolution: "gid://shopify/ProductVariant/48006450970846",
};

serve(async (req) => {
  if (req.method === "OPTIONS") {
    return new Response(null, { headers: corsHeaders });
  }

  try {
    const storeDomain = Deno.env.get("SHOPIFY_STORE_DOMAIN");
    const storefrontToken = Deno.env.get("SHOPIFY_STOREFRONT_ACCESS_TOKEN");
    if (!storeDomain || !storefrontToken) {
      throw new Error("Shopify configuration is missing");
    }

    const {
      resolution,
      styleId,
      customerEmail,
      chartData,
      artworkImageUrl,
      celestialOrderId,
      dtId,
    } = await req.json();

    const resolvedResolution =
      resolution === "standard" ? "standard" : "high_resolution";
    const merchandiseId = VARIANT_MAP[resolvedResolution];

    const lineAttributes = [
      { key: "_celestial_order_id", value: celestialOrderId || "" },
      { key: "_funnel_type", value: "digital" },
    ];

    const cartAttributes: { key: string; value: string }[] = [
      { key: "artwork_url", value: artworkImageUrl || "" },
      { key: "product_type", value: "digital" },
      { key: "resolution", value: resolvedResolution },
      { key: "style_id", value: styleId || "" },
      { key: "sun_sign", value: chartData?.sun?.sign || "" },
      { key: "moon_sign", value: chartData?.moon?.sign || "" },
      { key: "rising_sign", value: chartData?.rising || "" },
    ];

    if (dtId) {
      cartAttributes.push({ key: "dt_id", value: dtId });
    }

    const note = [
      "Digital Artwork",
      resolvedResolution,
      `Sun: ${chartData?.sun?.sign || "N/A"}`,
      `Moon: ${chartData?.moon?.sign || "N/A"}`,
      `Rising: ${chartData?.rising || "N/A"}`,
    ].join(" | ");

    const mutation = `
      mutation cartCreate($input: CartInput!) {
        cartCreate(input: $input) {
          cart {
            id
            checkoutUrl
          }
          userErrors {
            field
            message
          }
        }
      }
    `;

    const variables = {
      input: {
        lines: [
          {
            merchandiseId,
            quantity: 1,
            attributes: lineAttributes,
          },
        ],
        attributes: cartAttributes,
        note,
        buyerIdentity: customerEmail
          ? { email: customerEmail }
          : undefined,
      },
    };

    const shopifyResponse = await fetch(
      `https://${storeDomain}/api/2024-10/graphql.json`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-Shopify-Storefront-Access-Token": storefrontToken,
        },
        body: JSON.stringify({ query: mutation, variables }),
      }
    );

    if (!shopifyResponse.ok) {
      const text = await shopifyResponse.text();
      throw new Error(
        `Shopify API error (${shopifyResponse.status}): ${text}`
      );
    }

    const result = await shopifyResponse.json();
    const userErrors = result.data?.cartCreate?.userErrors;
    if (userErrors && userErrors.length > 0) {
      throw new Error(
        `Shopify cart error: ${userErrors.map((e: any) => e.message).join(", ")}`
      );
    }

    const checkoutUrl = result.data?.cartCreate?.cart?.checkoutUrl;
    if (!checkoutUrl) {
      throw new Error("No checkout URL returned from Shopify");
    }

    return new Response(JSON.stringify({ url: checkoutUrl }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
  } catch (error) {
    console.error("Digital checkout error:", error);
    return new Response(JSON.stringify({ error: error.message }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 500,
    });
  }
});
