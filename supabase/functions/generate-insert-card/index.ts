...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const svgContent = buildInsertCardSVG({
      customerName: firstName(customerName || "Your"),
      birthDate:    birthDate    || "",
      birthTime:    birthTime    || "",
...
function buildInsertCardSVG(data: InsertCardData): string {
  ...
  // Title: Erode Medium 102px — first name only
  y += 63;
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    // Section title: TASA Explorer — "The " regular + rest semibold
    y += 28 + 43;
    ...
    // Body: Erode Medium — wider wrap (68 chars), no extra trailing space
    y += 55 + 35;
    const bodyText = safeExplanation(sec.explanation || "");
    const lines = wrapLines(bodyText, 68);
