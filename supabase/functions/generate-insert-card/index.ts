...
// A5 @ 300dpi: 1748 x 2480px — all measurements match final approved Python mockup (v12)
const W       = 1748;
const H       = 2480;
const PAD_X   = 120;
const PAD_TOP = 110;

// ── SAFETY NET: hard-truncate any explanation over 300 chars ─────────────────
...
async function svgToPng(svgContent: string): Promise<Uint8Array> {
  try {
    ...
    return await svg2png(svgContent, { width: W, height: H });
  } catch (e) {
    console.error("svg2png failed, returning SVG fallback:", e);
...
function gradientRule(id: string, y: number): string {
  return `<rect x="${PAD_X}" y="${y}" width="${W - PAD_X * 2}" height="2"
    fill="url(#${id})" opacity="0.20"/>`;
}
...
function buildInsertCardSVG(data: InsertCardData): string {
  ...
  const CAP_TITLE = 63;
  y += CAP_TITLE;
  els += `<text x="${PAD_X}" y="${y}"
    font-family="Georgia, 'Times New Roman', serif"
    font-size="${FS_TITLE}" font-weight="500" fill="#FFFFFF"
    letter-spacing="-3">${esc(customerName)}'s Cosmic Blueprint</text>\n`;
  ...
  els += `<text x="${PAD_X}" y="${y}"
    ...
    font-size="${FS_META}" font-weight="700" fill="#FFFFFF"
    letter-spacing="1">${esc(metaText)}</text>\n`;
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    els += `<text x="${PAD_X}" y="${y}"
      font-family="'Arial', sans-serif"
      font-size="${FS_LABEL}" font-weight="600" fill="#FFFFFF"
    ...
    els += `<text x="${PAD_X}" y="${y}"
      font-family="'Arial', sans-serif"
      font-size="${FS_STITLE}" fill="#FFFFFF"
      letter-spacing="-1">${titleSvg}</text>\n`;
    ...
    for (let j = 0; j < lines.length; j++) {
      ...
      els += `<text x="${PAD_X}" y="${y}"
        font-family="Georgia, 'Times New Roman', serif"
        font-size="${FS_BODY}" font-weight="500" fill="#CCCCCC">${esc(lines[j])}</text>\n`;
    }
    y += CAP_BODY;
  }
  ...
  els += `<g transform="translate(${W - PAD_X - LOGO_W}, ${footerY})">
    ...
    <text x="95" y="62"
      font-family="'Arial', sans-serif"
      font-size="52" font-weight="600" fill="#BEBEBE"
      letter-spacing="-1">Celestial Artworks</text>
  </g>\n`;

  els += `<text x="${PAD_X}" y="${footerY + 57}"
    font-family="'Arial', sans-serif"
    font-size="${FS_FOOTER}" font-weight="500" fill="#BEBEBE">
    CelestialArtworks.com</text>\n`;
