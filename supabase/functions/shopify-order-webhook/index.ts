...
Deno.serve(async (req: Request) => {
  ...
  console.warn("DEPRECATED: shopify-order-webhook received a request. All new orders should be processed by woocommerce-order-webhook. This function is no longer processing orders.");

  // Still verify HMAC and parse body to avoid Shopify retries, but do not process.
  ...
  if (!await verifyShopifyHmac(webhookSecret, rawBody, hmacHeader)) {
    console.warn("webhook: Invalid HMAC (deprecated endpoint)");
  ...
  console.warn(`DEPRECATED: Ignored Shopify order ${order.id} (order_number=${order.order_number ?? order.name ?? "unknown"}). Migrate webhook to woocommerce-order-webhook.`);
